{{ config(
    ...
    pre_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, start_time, status, process_type, source_system, target_system, user_executed) VALUES (UUID_STRING(), 'go_quality_facts_load', CURRENT_TIMESTAMP(), 'STARTED', 'FACT_LOAD', 'SILVER', 'GOLD', CURRENT_USER()) WHERE '{{ this.name }}' != 'go_process_audit'",
    post_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, end_time, status, process_type, source_system, target_system, user_executed, records_processed) VALUES (UUID_STRING(), 'go_quality_facts_load', CURRENT_TIMESTAMP(), 'COMPLETED', 'FACT_LOAD', 'SILVER', 'GOLD', CURRENT_USER(), (SELECT COUNT(*) FROM {{ this }})) WHERE '{{ this.name }}' != 'go_process_audit'"
) }}
