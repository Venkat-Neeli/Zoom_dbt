{{ config(
    ...
    pre_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, start_time, status, process_type, user_executed, load_date) SELECT UUID_STRING(), 'go_usage_facts', CURRENT_TIMESTAMP(), 'STARTED', 'FACT_LOAD', 'DBT_USER', CURRENT_DATE() WHERE '{{ this.name }}' != 'go_process_audit'",
    post_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, end_time, status, records_processed, process_type, user_executed, load_date) SELECT UUID_STRING(), 'go_usage_facts', CURRENT_TIMESTAMP(), 'COMPLETED', (SELECT COUNT(*) FROM {{ this }}), 'FACT_LOAD', 'DBT_USER', CURRENT_DATE() WHERE '{{ this.name }}' != 'go_process_audit'"
) }}
...
usage_dates AS (
    ...
    WHERE record_status = 'ACTIVE'
        {% if is_incremental() %}
        AND usage_date > (SELECT MAX(usage_date) FROM {{ this }})
        {% endif %}
),
...
final_usage_facts AS (
    ...
    LEFT JOIN feature_usage_agg fua ON ub.user_id = fua.user_id AND ud.usage_date = fua.usage_date
