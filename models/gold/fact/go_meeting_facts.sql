{{ config(
    ...
    pre_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, start_time, status, source_system, target_system, process_type) VALUES (UUID_STRING(), 'go_meeting_facts', CURRENT_TIMESTAMP(), 'STARTED', 'SILVER', 'GOLD', 'FACT_LOAD')",
    post_hook="INSERT INTO {{ ref('go_process_audit') }} (execution_id, pipeline_name, end_time, status, records_processed, source_system, target_system, process_type) VALUES (UUID_STRING(), 'go_meeting_facts', CURRENT_TIMESTAMP(), 'COMPLETED', (SELECT COUNT(*) FROM {{ this }}), 'SILVER', 'GOLD', 'FACT_LOAD')"
) }}
...
feature_metrics AS (
    SELECT 
        fu.meeting_id,
        SUM(CASE WHEN fu.feature_name = 'Screen Sharing' THEN 1 ELSE 0 END) AS screen_share_count,
        ...
        MAX(CASE WHEN fu.feature_name = 'Recording' THEN TRUE ELSE FALSE END) AS recording_enabled
...
final_meeting_facts AS (
    SELECT 
        ...
        COALESCE(pm.max_concurrent_participants, 0) AS max_concurrent_participants,
